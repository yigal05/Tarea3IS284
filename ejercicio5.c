...
long long int FindPerrin(int n){
	// n representa el elemento que se esta buscando actualmente
	return (n ==0)? 3 : (n == 1)? 0 : (n==2)? 2 : FindPerrin(n-2)+ FindPerrin(n-3);
...
void KeepCycle(int i,int desiredTerms){
	...
	(i == desiredTerms)? printf("\b\b."): ( printf("%i, ", FindPerrin(i)) , KeepCycle(i+1,desiredTerms));
}//keepCycle es la funcion encargada de repetir una seria de intrucciones la veces que queramos

int main(){
	int desiredTerms=0; //desiredTerms guarda los terminos que el usuario desea ver
	printf("Ingrese numero de terminos deseados en la Serie de Perrin: ");
